...
while true; do
  RESPONSE=$(curl -sSLw "\n%{http_code}" -H "X-Vault-Token: super-secret-token" -H "Content-Type: application/json" -X POST -d @infra_config/vault/application.json http://$1:$2/v1/secret/data/application)
